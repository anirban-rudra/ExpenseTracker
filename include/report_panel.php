...
$select_income = "SELECT SUM(income_amt) FROM income WHERE income_month='$month' AND income_year='$year' AND user_id='$user_id'";
$run_income = mysqli_query($conn, $select_income);
    $row_income = mysqli_fetch_array($run_income);

    $total_income = $row_income['SUM(income_amt)'];
    
$select_expense = "SELECT SUM(expense_amt) FROM expense WHERE expense_month='$month' AND expense_year='$year' AND user_id='$user_id'";
